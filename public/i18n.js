window.__INTAHE_I18N__ = {
  fr: {
    common: {
      unknown_error: 'Erreur inconnue.',
    },
    discover: {
      empty: 'Aucun événement découvrable pour le moment.',
      loading: 'Chargement…',
      load_error: 'Impossible de charger les événements. ',
      geo_unavailable: 'La géolocalisation n’est pas disponible sur cet appareil.',
      locating: 'Localisation en cours…',
      location_denied: 'Position refusée — affichage de tous les événements à venir.',
    },
    event: {
      load_error: 'Impossible de charger cet événement. ',
      tickets_heading: 'Billets',
      tickets_empty: 'Aucun billet disponible pour cet événement.',
      remaining: '{{n}} restant(s)',
      share_button: 'Partager',
      share_copied: 'Lien copié !',
      email_label: 'E-mail',
      quantity_label: 'Quantité',
      order_button: 'Commander',
      order_button_wait: 'Un instant…',
      missing_fields: 'Renseigne ton e-mail et une quantité valide.',
      order_summary_subtotal: 'Sous-total : {{amount}}',
      order_summary_tax_line: '{{label}} ({{rate}}%) : {{amount}}',
      order_summary_fees: 'Frais de service : {{amount}}',
      order_summary_total: 'Total à payer : {{amount}}',
      payment_not_ready: 'Le paiement n’a pas pu être initialisé.',
      pay_button: 'Payer',
      pay_button_wait: 'Paiement en cours…',
      payment_failed: 'Le paiement a échoué.',
      payment_confirming: 'Paiement réussi. Préparation de tes billets…',
      payment_succeeded: 'Paiement réussi ! Consulte tes courriels : tes billets t’y attendent.',
      payment_form_load_error:
        'Le formulaire de paiement n’a pas pu se charger. Vérifie ta connexion et recharge la page.',
      payment_taking_long: 'Le paiement prend plus de temps que prévu. Ne ferme pas cette page — patiente encore un peu.',
      request_timeout: 'La demande a pris trop de temps. Vérifie ta connexion et réessaie.',
      title_suffix: ' — Intahé',
    },
    tickets: {
      title: 'Mes billets',
      load_error:
        'Impossible de charger les billets (le paiement n’a peut-être pas encore été confirmé — réessaie dans quelques secondes). ',
      empty:
        'Aucun billet pour l’instant — si tu viens de payer, le paiement est peut-être encore en cours de confirmation. Recharge la page dans quelques secondes.',
      qr_alt: 'QR code du billet',
      scanned: 'Scanné',
      not_scanned: 'Pas encore scanné',
    },
    login: {
      brand: 'Intahé',
      subtitle: 'Connecte-toi pour gérer tes événements ou tes billets.',
      email: 'Email',
      password: 'Mot de passe',
      submit: 'Se connecter',
      submit_wait: 'Un instant…',
      invalid_credentials: 'Email ou mot de passe incorrect.',
      no_account: 'Pas encore de compte ? Inscris-toi',
      discover_link: 'Découvrir des événements sans compte',
      error_generic: 'Une erreur est survenue. Réessaie.',
    },
    signup: {
      title: 'Créer un compte',
      subtitle: 'Un seul compte pour acheter des billets et organiser des événements.',
      full_name: 'Nom complet',
      email: 'Email',
      password: 'Mot de passe',
      submit: 'Créer le compte',
      submit_wait: 'Un instant…',
      email_taken: 'Un compte existe déjà avec cet email.',
      already_account: 'Déjà un compte ? Connecte-toi',
      error_generic: 'Une erreur est survenue. Réessaie.',
    },
    roles: {
      owner: 'Propriétaire',
      admin: 'Admin',
      staff: 'Staff',
      volunteer: 'Bénévole',
    },
    event_status: {
      draft: 'Brouillon',
      published: 'Publié',
      cancelled: 'Annulé',
      completed: 'Terminé',
    },
    organizations_list: {
      title: 'Organisations',
      discover_button: 'Découvrir des événements',
      pending_invites_title: 'Invitations en attente',
      accept_button: 'Accepter',
      org_name_label: 'Nom de l’organisation',
      cancel_button: 'Annuler',
      create_button: 'Créer',
      new_org_button: 'Nouvelle organisation',
      empty: 'Aucune organisation pour l’instant.',
      load_error: 'Impossible de charger les organisations.',
      create_error: 'Impossible de créer l’organisation.',
      accept_error: 'Impossible d’accepter cette invitation.',
    },
    organization_detail: {
      members_button: 'Membres',
      dashboard_button: 'Tableau de bord',
      payouts_button: 'Versements',
      quick_sale_button: 'Vente rapide',
      event_name_label: 'Nom de l’événement',
      start_label: 'Début',
      end_label: 'Fin',
      address_label: 'Adresse (optionnel)',
      use_current_location: 'Utiliser ma position actuelle',
      location_saved: 'Position enregistrée ✓',
      geo_unavailable: 'La géolocalisation n’est pas disponible sur cet appareil.',
      discoverable_title: 'Événement découvrable',
      discoverable_subtitle: 'Visible dans la recherche publique d’événements à proximité.',
      cancel_button: 'Annuler',
      create_button: 'Créer',
      new_event_button: 'Nouvel événement',
      empty: 'Aucun événement pour l’instant.',
      load_error: 'Impossible de charger cette organisation.',
      create_event_error: 'Impossible de créer l’événement.',
    },
    org_members: {
      invite_title: 'Inviter un membre',
      email_label: 'Email',
      invite_button: 'Inviter',
      empty: 'Aucun membre pour l’instant.',
      pending_invite_suffix: ' (invitation en attente)',
      change_role_button: 'Changer le rôle',
      remove_button: 'Retirer',
      load_error: 'Impossible de charger les membres.',
      invite_error_generic: 'Impossible d’inviter cette personne.',
      invite_error_not_found: 'Aucun compte Intahé n’existe avec cet email.',
      invite_error_already_member: 'Cette personne est déjà membre de l’organisation.',
      invite_error_already_pending: 'Une invitation est déjà en attente pour cette personne.',
      role_update_error: 'Impossible de modifier ce rôle.',
      remove_error: 'Impossible de retirer ce membre.',
    },
    org_dashboard: {
      tickets_sold: 'Billets vendus',
      orders_paid: 'Commandes payées',
      net_revenue: 'Revenu net',
      by_event: 'Par événement',
      empty: 'Aucun événement pour l’instant.',
      orders_tickets_summary: '{{orders}} commandes · {{tickets}} billets',
      net_prefix: 'Net : {{amount}}',
      load_error: 'Impossible de charger le tableau de bord.',
      capacity_exceeded_badge: 'Capacité dépassée de {{n}}',
      capacity_exceeded_view_details: 'Voir les commandes concernées',
      capacity_exceeded_hide_details: 'Masquer',
      capacity_exceeded_load_error: 'Impossible de charger le détail.',
      capacity_incident_line: '{{ticket_type}} — commande {{order}} ({{email}}), {{sold}}/{{total}} vendus',
    },
    org_stripe: {
      section_title: 'Facturation Stripe',
      intro:
        'Stripe vérifie ton identité pour que l’argent aille directement dans ton compte, sans passer par nous — environ 10 minutes, prépare ton numéro d’entreprise et un relevé bancaire.',
      connect_button: 'Connecter Stripe',
      status_not_connected:
        'Stripe n’est pas connecté. La vente de billets payants est bloquée tant que ce n’est pas fait — un événement entièrement gratuit peut être publié et vendu sans Stripe.',
      status_pending_title: 'Vérification Stripe en cours',
      status_pending_body:
        'Stripe vérifie encore ton compte — ça peut prendre quelques minutes après avoir complété le formulaire. Reviens ici et rafraîchis le statut.',
      status_active: 'Stripe est connecté et actif — tu peux vendre des billets payants.',
      refresh_button: 'Rafraîchir le statut',
      connect_error: 'Impossible de démarrer la connexion Stripe.',
      load_error: 'Impossible de charger le statut Stripe.',
    },
    org_tax: {
      section_title: 'Taxes de vente',
      intro:
        'Désactivée par défaut — aucune taxe n’est ajoutée tant qu’aucune ligne n’est configurée ici. Ajoute une ligne par taxe à percevoir (ex. TPS 5 %, TVQ 9,975 %) une fois inscrit auprès de l’autorité fiscale concernée.',
      empty: 'Aucune taxe configurée — les prix affichés sont les prix finaux.',
      label_placeholder: 'Ex. TPS',
      rate_placeholder: 'Taux %',
      remove_line: 'Retirer',
      add_line: 'Ajouter une ligne de taxe',
      save_button: 'Enregistrer',
      save_success: 'Configuration des taxes enregistrée.',
      save_error: 'Impossible d’enregistrer la configuration des taxes.',
      load_error: 'Impossible de charger la configuration des taxes.',
      validation_label_required: 'Chaque ligne doit avoir un nom.',
      validation_rate_invalid: 'Le taux doit être un nombre entre 0 et 100.',
    },
    org_payouts: {
      title: 'Versements',
      back_button: 'Retour à l’organisation',
      collected_balance_label: 'Solde encaissé',
      collected_balance_hint: 'Encaissé par Stripe, pas encore disponible pour versement.',
      available_balance_label: 'Solde disponible',
      available_balance_hint: 'Prêt à être versé à ton compte bancaire.',
      not_connected: 'Connecte Stripe pour voir tes versements.',
      upcoming_title: 'Versements à venir',
      upcoming_empty: 'Aucun versement à venir.',
      next_payout_line: '{{event}} — prévu le {{date}}',
      delay_note: '48 h après la fin de l’événement.',
      history_title: 'Historique des versements',
      history_empty: 'Aucun versement pour l’instant.',
      status_succeeded: 'Exécuté',
      status_failed: 'En échec',
      status_skipped_no_balance: 'Reporté (aucun solde disponible)',
      status_pending: 'En cours',
      amount_prefix: 'Montant : {{amount}}',
      error_prefix: 'Erreur : {{message}}',
      load_error: 'Impossible de charger les versements.',
    },
    quick_sale: {
      title: 'Vente rapide',
      back_button: 'Retour à l’organisation',
      intro: 'Vends un produit ou service à prix fixe et encaisse directement.',
      catalog_title: 'Catalogue',
      catalog_empty: 'Aucun produit pour l’instant — ajoutes-en un ci-dessous.',
      add_item_button: 'Ajouter',
      item_name_label: 'Nom',
      item_price_label: 'Prix',
      item_currency_cad: 'CAD',
      item_currency_usd: 'USD',
      create_item_error: 'Impossible de créer ce produit.',
      delete_item_confirm: 'Supprimer ce produit du catalogue ?',
      delete_item_error: 'Impossible de supprimer ce produit.',
      sell_button: 'Vendre',
      sell_button_wait: 'Un instant…',
      sell_error: 'Impossible de créer la vente.',
      pay_button: 'Payer',
      pay_button_wait: 'Paiement en cours…',
      payment_failed: 'Le paiement a échoué.',
      payment_succeeded: 'Paiement réussi.',
      payment_not_ready: 'Le paiement n’a pas pu être initialisé.',
      recent_sales_title: 'Ventes récentes',
      recent_sales_empty: 'Aucune vente pour l’instant.',
      status_pending: 'En attente',
      status_paid: 'Payée',
      status_failed: 'Échouée',
      payout_status_succeeded: 'Versement instantané réussi',
      payout_status_failed: 'Versement en échec',
      payout_status_not_attempted: 'Versement en attente',
      retry_payout_button: 'Réessayer le versement',
      retry_payout_error: 'Le versement a encore échoué.',
      load_error: 'Impossible de charger la vente rapide.',
    },
    event_fees: {
      title: 'Détail des frais',
      back_button: 'Retour à l’événement',
      per_ticket_title: 'Par type de billet',
      price_label: 'Prix',
      commission_label: 'Commission Intahé',
      sold_label: '{{n}} vendus',
      gross_label: 'Brut : {{amount}}',
      cumulative_title: 'Cumulé',
      tickets_sold_label: 'Billets vendus',
      gross_revenue_label: 'Revenu brut',
      stripe_fees_label: 'Frais Stripe',
      intahe_fees_label: 'Commission Intahé',
      net_revenue_label: 'Net organisateur',
      empty: 'Aucun type de billet pour l’instant.',
      load_error: 'Impossible de charger le détail des frais.',
    },
    stripe_connect_return: {
      message: 'Merci ! Retour à ton organisation…',
      no_org_message: 'Impossible de retrouver l’organisation.',
      organizations_link: 'Voir mes organisations',
    },
    admin_payouts: {
      title: 'Console d’administration — Versements',
      forbidden: 'Accès refusé — réservé au personnel Intahé.',
      due_title: 'Versements dus',
      due_empty: 'Aucun versement dû.',
      executed_title: 'Versements exécutés',
      executed_empty: 'Aucun versement exécuté.',
      failed_title: 'Versements en échec',
      failed_empty: 'Aucun versement en échec.',
      overdue_label: 'En retard de {{n}} h',
      held_badge: 'Retenu',
      hold_button: 'Retenir',
      unhold_button: 'Lever la retenue',
      trigger_button: 'Déclencher maintenant',
      trigger_confirm: 'Déclencher un versement maintenant pour cet événement ?',
      action_error: 'Action impossible.',
      load_error: 'Impossible de charger la console d’administration.',
      amount_prefix: 'Montant : {{amount}}',
      error_prefix: 'Erreur : {{message}}',
      reconciliation_link: 'Réconciliation des paiements bloqués →',
    },
    admin_reconciliation: {
      title: 'Console d’administration — Réconciliation',
      payouts_link: '← Versements',
      intro:
        'Commandes où l’acheteur a été débité chez Stripe mais dont le billet n’a jamais été émis — le webhook de confirmation n’est jamais arrivé ou a échoué.',
      forbidden: 'Accès refusé — réservé au personnel Intahé.',
      open_title: 'Incidents ouverts',
      open_empty: 'Aucun incident ouvert.',
      stuck_badge: 'Paiement bloqué',
      incident_detail: 'Acheteur : {{buyer}} · Montant débité : {{amount}} · Détecté : {{detected}}',
      reconcile_button: 'Réémettre les billets',
      reconcile_confirm:
        'Réémettre les billets de cette commande sans nouveau paiement ? Le paiement a déjà été confirmé réussi chez Stripe.',
      action_error: 'Réconciliation impossible.',
      load_error: 'Impossible de charger la console de réconciliation.',
      resolved_title: 'Incidents résolus',
      resolved_empty: 'Aucun incident résolu.',
      resolved_detail: '{{amount}} · {{resolution}} · {{date}}',
      resolution_manual: 'Billets réémis manuellement',
      resolution_webhook: 'Confirmation arrivée en retard',
    },
    manage_event: {
      load_error: 'Impossible de charger l’événement.',
      action_error: 'Action impossible.',
      create_ticket_type_error: 'Impossible de créer ce type de billet.',
      date_range: 'Du {{start}} au {{end}}',
      publish_button: 'Publier l’événement',
      cancel_event_button: 'Annuler l’événement',
      management_title: 'Gestion',
      orders_button: 'Commandes',
      guest_list_button: 'Liste des invités',
      check_in_button: 'Check-in',
      fees_button: 'Frais',
      cancel_confirm_message:
        'Les frais Stripe déjà prélevés sur les commandes ne sont jamais remboursés par Stripe — ils resteront à la charge de ton organisation. Confirmer l’annulation de l’événement ?',
      ticket_type_stripe_required: 'Connecte un compte Stripe actif avant de créer un type de billet payant.',
      ticket_types_title: 'Types de billets',
      add_button: 'Ajouter',
      ticket_name_label: 'Nom',
      ticket_price_label: 'Prix (CAD)',
      price_estimate:
        'Commission Intahé : {{commission}} · Frais Stripe estimés : {{stripe_fee}} · Net organisateur estimé : {{net}}',
      price_estimate_free: 'Billet gratuit — aucun frais.',
      price_below_minimum:
        'Le prix minimum est de {{minimum}}. En dessous, les frais de traitement dépasseraient ce que tu reçois. Pour un événement sans frais, mets le prix à 0 $.',
      price_below_minimum_error:
        'Le prix minimum est de {{minimum}}. En dessous, les frais de traitement dépasseraient ce que tu reçois. Pour un événement sans frais, mets le prix à 0 $.',
      ticket_quantity_label: 'Quantité disponible',
      create_type_button: 'Créer',
      cancel_form_button: 'Annuler',
      ticket_types_empty: 'Aucun type de billet pour l’instant.',
      sold_summary: '{{price}} · {{sold}}/{{total}} vendus',
      selected: 'Sélectionné',
      buy_tickets_title: 'Acheter des billets',
      buy_tickets_hint: 'Choisis un type de billet ci-dessus, puis renseigne les informations pour créer la commande.',
      order_created: 'Commande créée',
      total: 'Total : {{amount}}',
      status: 'Statut : {{status}}',
      payment_succeeded: 'Paiement réussi',
      view_tickets: 'Voir les billets',
    },
    check_in: {
      hint: 'Saisis le code du billet (habituellement scanné via QR code) pour l’enregistrer à l’entrée.',
      code_label: 'Code du billet',
      submit_button: 'Enregistrer l’entrée',
      error_generic: 'Impossible de valider ce billet.',
      error_not_found: 'Aucun billet ne correspond à ce code pour cet événement.',
      error_already: 'Ce billet a déjà été scanné.',
      success_title: 'Billet validé',
      capacity_warning: 'Ce type de billet dépasse sa capacité de {{n}} — billet valide, quand même accepté.',
    },
    guest_list: {
      empty: 'Aucun billet vendu pour l’instant.',
      scanned: 'Scanné',
      pending: 'En attente',
      load_error: 'Impossible de charger la liste des invités.',
    },
    org_orders: {
      empty: 'Aucune commande pour l’instant.',
      status_pending: 'En attente',
      status_paid: 'Payée',
      status_refunded: 'Remboursée',
      status_partial_refund: 'Partiellement remboursée',
      status_expired: 'Expirée',
      load_error: 'Impossible de charger les commandes.',
      refund_button: 'Rembourser',
      refund_amount_label: 'Montant',
      refund_amount_full: 'Total',
      refund_amount_partial: 'Partiel',
      refund_amount_partial_input_label: 'Montant partiel',
      refund_reason_label: 'Motif du remboursement',
      refund_reason_placeholder: 'Choisis un motif',
      refund_reason_cancel: 'J’annule l’événement',
      refund_reason_postpone: 'Je reporte l’événement',
      refund_reason_buyer_request: 'L’acheteur en fait la demande',
      refund_reason_required: 'Choisis un motif.',
      commission_refunded_note: 'La commission Intahé sera remboursée à l’acheteur.',
      commission_not_refunded_note: 'La commission Intahé ne sera pas remboursée — le service a été rendu.',
      refund_continue_button: 'Continuer',
      refund_cancel_button: 'Annuler',
      refund_confirm_title: 'Confirmer le remboursement',
      refund_confirm_amount_prefix: 'Montant remboursé à l’acheteur : {{amount}}',
      stripe_fees_warning:
        'Les frais Stripe ne sont jamais remboursés par Stripe — ils resteront à la charge de ton organisation.',
      refund_confirm_button: 'Confirmer le remboursement',
      refund_back_button: 'Retour',
      refund_error: 'Impossible d’effectuer ce remboursement.',
      refunded_on_prefix: 'Remboursée le {{date}} — motif : {{reason}}',
    },
    profile: {
      logout: 'Se déconnecter',
      delete_account_link: 'Supprimer mon compte',
    },
    delete_account: {
      title: 'Supprimer mon compte',
      warning:
        'Cette action est irréversible. Ton profil sera supprimé définitivement. Tes commandes et billets passés restent visibles aux organisateurs concernés, comme l’exige la tenue de registres.',
      password_label: 'Mot de passe',
      password_required: 'Entre ton mot de passe pour confirmer.',
      confirm_button: 'Supprimer définitivement mon compte',
      invalid_password: 'Mot de passe incorrect.',
      owns_organizations: 'Transfère ou supprime ton (tes) organisation(s) avant de supprimer ton compte.',
      error_generic: 'Une erreur est survenue. Réessaie.',
      confirm_dialog_message: 'Cette action est définitive et ne peut pas être annulée. Confirmer la suppression ?',
    },
  },
  en: {
    common: {
      unknown_error: 'Unknown error.',
    },
    discover: {
      empty: 'No discoverable events right now.',
      loading: 'Loading…',
      load_error: 'Unable to load events. ',
      geo_unavailable: 'Geolocation is not available on this device.',
      locating: 'Locating…',
      location_denied: 'Location denied — showing all upcoming events.',
    },
    event: {
      load_error: 'Unable to load this event. ',
      tickets_heading: 'Tickets',
      tickets_empty: 'No tickets available for this event.',
      remaining: '{{n}} left',
      share_button: 'Share',
      share_copied: 'Link copied!',
      email_label: 'Email',
      quantity_label: 'Quantity',
      order_button: 'Order',
      order_button_wait: 'One moment…',
      missing_fields: 'Enter your email and a valid quantity.',
      order_summary_subtotal: 'Subtotal: {{amount}}',
      order_summary_tax_line: '{{label}} ({{rate}}%): {{amount}}',
      order_summary_fees: 'Service fee: {{amount}}',
      order_summary_total: 'Total due: {{amount}}',
      payment_not_ready: 'Payment could not be initialized.',
      pay_button: 'Pay',
      pay_button_wait: 'Processing payment…',
      payment_failed: 'Payment failed.',
      payment_confirming: 'Payment successful. Preparing your tickets…',
      payment_succeeded: 'Payment successful! Check your email — your tickets are on their way.',
      payment_form_load_error: 'The payment form could not load. Check your connection and reload the page.',
      payment_taking_long: "Payment is taking longer than usual. Don't close this page — hang tight a bit longer.",
      request_timeout: 'The request took too long. Check your connection and try again.',
      title_suffix: ' — Intahé',
    },
    tickets: {
      title: 'My tickets',
      load_error:
        'Unable to load tickets (the payment may not be confirmed yet — try again in a few seconds). ',
      empty:
        'No tickets yet — if you just paid, the payment may still be confirming. Reload the page in a few seconds.',
      qr_alt: 'Ticket QR code',
      scanned: 'Scanned',
      not_scanned: 'Not scanned yet',
    },
    login: {
      brand: 'Intahé',
      subtitle: 'Sign in to manage your events or tickets.',
      email: 'Email',
      password: 'Password',
      submit: 'Sign in',
      submit_wait: 'One moment…',
      invalid_credentials: 'Incorrect email or password.',
      no_account: "Don't have an account? Sign up",
      discover_link: 'Discover events without an account',
      error_generic: 'Something went wrong. Try again.',
    },
    signup: {
      title: 'Create an account',
      subtitle: 'One account to buy tickets and organize events.',
      full_name: 'Full name',
      email: 'Email',
      password: 'Password',
      submit: 'Create account',
      submit_wait: 'One moment…',
      email_taken: 'An account already exists with this email.',
      already_account: 'Already have an account? Sign in',
      error_generic: 'Something went wrong. Try again.',
    },
    roles: {
      owner: 'Owner',
      admin: 'Admin',
      staff: 'Staff',
      volunteer: 'Volunteer',
    },
    event_status: {
      draft: 'Draft',
      published: 'Published',
      cancelled: 'Cancelled',
      completed: 'Completed',
    },
    organizations_list: {
      title: 'Organizations',
      discover_button: 'Discover events',
      pending_invites_title: 'Pending invites',
      accept_button: 'Accept',
      org_name_label: 'Organization name',
      cancel_button: 'Cancel',
      create_button: 'Create',
      new_org_button: 'New organization',
      empty: 'No organizations yet.',
      load_error: 'Unable to load organizations.',
      create_error: 'Unable to create the organization.',
      accept_error: 'Unable to accept this invite.',
    },
    organization_detail: {
      members_button: 'Members',
      dashboard_button: 'Dashboard',
      payouts_button: 'Payouts',
      quick_sale_button: 'Quick sale',
      event_name_label: 'Event name',
      start_label: 'Start',
      end_label: 'End',
      address_label: 'Address (optional)',
      use_current_location: 'Use my current location',
      location_saved: 'Location saved ✓',
      geo_unavailable: 'Geolocation is not available on this device.',
      discoverable_title: 'Discoverable event',
      discoverable_subtitle: 'Visible in the public search for nearby events.',
      cancel_button: 'Cancel',
      create_button: 'Create',
      new_event_button: 'New event',
      empty: 'No events yet.',
      load_error: 'Unable to load this organization.',
      create_event_error: 'Unable to create the event.',
    },
    org_members: {
      invite_title: 'Invite a member',
      email_label: 'Email',
      invite_button: 'Invite',
      empty: 'No members yet.',
      pending_invite_suffix: ' (invite pending)',
      change_role_button: 'Change role',
      remove_button: 'Remove',
      load_error: 'Unable to load members.',
      invite_error_generic: 'Unable to invite this person.',
      invite_error_not_found: 'No Intahé account exists with this email.',
      invite_error_already_member: 'This person is already a member of the organization.',
      invite_error_already_pending: 'An invite is already pending for this person.',
      role_update_error: 'Unable to change this role.',
      remove_error: 'Unable to remove this member.',
    },
    org_dashboard: {
      tickets_sold: 'Tickets sold',
      orders_paid: 'Paid orders',
      net_revenue: 'Net revenue',
      by_event: 'By event',
      empty: 'No events yet.',
      orders_tickets_summary: '{{orders}} orders · {{tickets}} tickets',
      net_prefix: 'Net: {{amount}}',
      load_error: 'Unable to load the dashboard.',
      capacity_exceeded_badge: 'Capacity exceeded by {{n}}',
      capacity_exceeded_view_details: 'View affected orders',
      capacity_exceeded_hide_details: 'Hide',
      capacity_exceeded_load_error: 'Unable to load the details.',
      capacity_incident_line: '{{ticket_type}} — order {{order}} ({{email}}), {{sold}}/{{total}} sold',
    },
    org_stripe: {
      section_title: 'Stripe billing',
      intro:
        "Stripe verifies your identity so the money goes straight into your account, never through us — about 10 minutes, have your business number and a bank statement ready.",
      connect_button: 'Connect Stripe',
      status_not_connected:
        "Stripe isn't connected. Selling paid tickets is blocked until this is done — a fully free event can be published and sold with no Stripe connection needed.",
      status_pending_title: 'Stripe verification in progress',
      status_pending_body:
        "Stripe is still verifying your account — this can take a few minutes after completing the form. Come back here and refresh the status.",
      status_active: 'Stripe is connected and active — you can sell paid tickets.',
      refresh_button: 'Refresh status',
      connect_error: 'Unable to start connecting Stripe.',
      load_error: 'Unable to load Stripe status.',
    },
    org_tax: {
      section_title: 'Sales tax',
      intro:
        "Off by default — no tax is added until a line is configured here. Add one line per tax you need to collect (e.g. GST 5%, QST 9.975%) once you're registered with the relevant tax authority.",
      empty: 'No tax configured — displayed prices are final prices.',
      label_placeholder: 'e.g. GST',
      rate_placeholder: 'Rate %',
      remove_line: 'Remove',
      add_line: 'Add a tax line',
      save_button: 'Save',
      save_success: 'Tax configuration saved.',
      save_error: 'Unable to save the tax configuration.',
      load_error: 'Unable to load the tax configuration.',
      validation_label_required: 'Every line needs a name.',
      validation_rate_invalid: 'Rate must be a number between 0 and 100.',
    },
    org_payouts: {
      title: 'Payouts',
      back_button: 'Back to organization',
      collected_balance_label: 'Collected balance',
      collected_balance_hint: 'Collected by Stripe, not yet available for payout.',
      available_balance_label: 'Available balance',
      available_balance_hint: 'Ready to be paid out to your bank account.',
      not_connected: 'Connect Stripe to see your payouts.',
      upcoming_title: 'Upcoming payouts',
      upcoming_empty: 'No upcoming payouts.',
      next_payout_line: '{{event}} — expected on {{date}}',
      delay_note: '48h after the event ends.',
      history_title: 'Payout history',
      history_empty: 'No payouts yet.',
      status_succeeded: 'Executed',
      status_failed: 'Failed',
      status_skipped_no_balance: 'Deferred (no balance available)',
      status_pending: 'In progress',
      amount_prefix: 'Amount: {{amount}}',
      error_prefix: 'Error: {{message}}',
      load_error: 'Unable to load payouts.',
    },
    quick_sale: {
      title: 'Quick sale',
      back_button: 'Back to organization',
      intro: 'Sell a fixed-price product or service and take payment on the spot.',
      catalog_title: 'Catalog',
      catalog_empty: 'No products yet — add one below.',
      add_item_button: 'Add',
      item_name_label: 'Name',
      item_price_label: 'Price',
      item_currency_cad: 'CAD',
      item_currency_usd: 'USD',
      create_item_error: 'Unable to create this product.',
      delete_item_confirm: 'Remove this product from the catalog?',
      delete_item_error: 'Unable to remove this product.',
      sell_button: 'Sell',
      sell_button_wait: 'One moment…',
      sell_error: 'Unable to create this sale.',
      pay_button: 'Pay',
      pay_button_wait: 'Processing payment…',
      payment_failed: 'Payment failed.',
      payment_succeeded: 'Payment successful.',
      payment_not_ready: 'Payment could not be initialized.',
      recent_sales_title: 'Recent sales',
      recent_sales_empty: 'No sales yet.',
      status_pending: 'Pending',
      status_paid: 'Paid',
      status_failed: 'Failed',
      payout_status_succeeded: 'Instant payout succeeded',
      payout_status_failed: 'Payout failed',
      payout_status_not_attempted: 'Payout pending',
      retry_payout_button: 'Retry payout',
      retry_payout_error: 'The payout failed again.',
      load_error: 'Unable to load the quick sale page.',
    },
    event_fees: {
      title: 'Fee breakdown',
      back_button: 'Back to event',
      per_ticket_title: 'By ticket type',
      price_label: 'Price',
      commission_label: 'Intahé commission',
      sold_label: '{{n}} sold',
      gross_label: 'Gross: {{amount}}',
      cumulative_title: 'Cumulative',
      tickets_sold_label: 'Tickets sold',
      gross_revenue_label: 'Gross revenue',
      stripe_fees_label: 'Stripe fees',
      intahe_fees_label: 'Intahé commission',
      net_revenue_label: 'Organizer net',
      empty: 'No ticket types yet.',
      load_error: 'Unable to load the fee breakdown.',
    },
    stripe_connect_return: {
      message: 'Thanks! Returning to your organization…',
      no_org_message: 'Unable to find the organization.',
      organizations_link: 'View my organizations',
    },
    admin_payouts: {
      title: 'Admin console — Payouts',
      forbidden: 'Access denied — Intahé staff only.',
      due_title: 'Due payouts',
      due_empty: 'No payouts due.',
      executed_title: 'Executed payouts',
      executed_empty: 'No payouts executed.',
      failed_title: 'Failed payouts',
      failed_empty: 'No failed payouts.',
      overdue_label: 'Overdue by {{n}}h',
      held_badge: 'Held',
      hold_button: 'Hold',
      unhold_button: 'Release hold',
      trigger_button: 'Trigger now',
      trigger_confirm: 'Trigger a payout now for this event?',
      action_error: 'This action failed.',
      load_error: 'Unable to load the admin console.',
      amount_prefix: 'Amount: {{amount}}',
      error_prefix: 'Error: {{message}}',
      reconciliation_link: 'Stuck payment reconciliation →',
    },
    admin_reconciliation: {
      title: 'Admin console — Reconciliation',
      payouts_link: '← Payouts',
      intro:
        'Orders where the buyer was charged at Stripe but no ticket was ever issued — the confirmation webhook never arrived, or failed.',
      forbidden: 'Access denied — Intahé staff only.',
      open_title: 'Open incidents',
      open_empty: 'No open incidents.',
      stuck_badge: 'Payment stuck',
      incident_detail: 'Buyer: {{buyer}} · Amount charged: {{amount}} · Detected: {{detected}}',
      reconcile_button: 'Reissue tickets',
      reconcile_confirm:
        "Reissue this order's tickets with no new payment? The payment has already been confirmed as succeeded at Stripe.",
      action_error: 'Reconciliation failed.',
      load_error: 'Unable to load the reconciliation console.',
      resolved_title: 'Resolved incidents',
      resolved_empty: 'No resolved incidents.',
      resolved_detail: '{{amount}} · {{resolution}} · {{date}}',
      resolution_manual: 'Tickets reissued manually',
      resolution_webhook: 'Confirmation arrived late',
    },
    manage_event: {
      load_error: 'Unable to load the event.',
      action_error: 'This action failed.',
      create_ticket_type_error: 'Unable to create this ticket type.',
      date_range: '{{start}} to {{end}}',
      publish_button: 'Publish event',
      cancel_event_button: 'Cancel event',
      management_title: 'Management',
      orders_button: 'Orders',
      guest_list_button: 'Guest list',
      check_in_button: 'Check-in',
      fees_button: 'Fees',
      cancel_confirm_message:
        "Stripe's fees already charged on orders are never refunded by Stripe — they'll stay on your organization's account. Confirm cancelling this event?",
      ticket_type_stripe_required: 'Connect an active Stripe account before creating a paid ticket type.',
      ticket_types_title: 'Ticket types',
      add_button: 'Add',
      ticket_name_label: 'Name',
      ticket_price_label: 'Price (CAD)',
      price_estimate:
        'Intahé commission: {{commission}} · Estimated Stripe fee: {{stripe_fee}} · Estimated organizer net: {{net}}',
      price_estimate_free: 'Free ticket — no fees.',
      price_below_minimum:
        'The minimum price is {{minimum}}. Below that, processing fees would exceed what you receive. For a free event, set the price to $0.',
      price_below_minimum_error:
        'The minimum price is {{minimum}}. Below that, processing fees would exceed what you receive. For a free event, set the price to $0.',
      ticket_quantity_label: 'Quantity available',
      create_type_button: 'Create',
      cancel_form_button: 'Cancel',
      ticket_types_empty: 'No ticket types yet.',
      sold_summary: '{{price}} · {{sold}}/{{total}} sold',
      selected: 'Selected',
      buy_tickets_title: 'Buy tickets',
      buy_tickets_hint: 'Choose a ticket type above, then fill in the details to create the order.',
      order_created: 'Order created',
      total: 'Total: {{amount}}',
      status: 'Status: {{status}}',
      payment_succeeded: 'Payment succeeded',
      view_tickets: 'View tickets',
    },
    check_in: {
      hint: 'Enter the ticket code (usually scanned via QR code) to check it in at the door.',
      code_label: 'Ticket code',
      submit_button: 'Check in',
      error_generic: 'Unable to validate this ticket.',
      error_not_found: 'No ticket matches this code for this event.',
      error_already: 'This ticket has already been scanned.',
      success_title: 'Ticket validated',
      capacity_warning: 'This ticket type is {{n}} over capacity — valid ticket, still accepted.',
    },
    guest_list: {
      empty: 'No tickets sold yet.',
      scanned: 'Scanned',
      pending: 'Pending',
      load_error: 'Unable to load the guest list.',
    },
    org_orders: {
      empty: 'No orders yet.',
      status_pending: 'Pending',
      status_paid: 'Paid',
      status_refunded: 'Refunded',
      status_partial_refund: 'Partially refunded',
      status_expired: 'Expired',
      load_error: 'Unable to load orders.',
      refund_button: 'Refund',
      refund_amount_label: 'Amount',
      refund_amount_full: 'Full',
      refund_amount_partial: 'Partial',
      refund_amount_partial_input_label: 'Partial amount',
      refund_reason_label: 'Refund reason',
      refund_reason_placeholder: 'Choose a reason',
      refund_reason_cancel: "I'm cancelling the event",
      refund_reason_postpone: "I'm postponing the event",
      refund_reason_buyer_request: 'The buyer requested it',
      refund_reason_required: 'Choose a reason.',
      commission_refunded_note: "Intahé's commission will be refunded to the buyer.",
      commission_not_refunded_note: "Intahé's commission will not be refunded — the service was rendered.",
      refund_continue_button: 'Continue',
      refund_cancel_button: 'Cancel',
      refund_confirm_title: 'Confirm the refund',
      refund_confirm_amount_prefix: 'Amount refunded to the buyer: {{amount}}',
      stripe_fees_warning:
        "Stripe's fees are never refunded by Stripe — they'll stay on your organization's account.",
      refund_confirm_button: 'Confirm the refund',
      refund_back_button: 'Back',
      refund_error: 'Unable to process this refund.',
      refunded_on_prefix: 'Refunded on {{date}} — reason: {{reason}}',
    },
    profile: {
      logout: 'Log out',
      delete_account_link: 'Delete my account',
    },
    delete_account: {
      title: 'Delete my account',
      warning:
        "This can't be undone. Your profile will be permanently deleted. Your past orders and tickets stay visible to the relevant organizers, as required for record-keeping.",
      password_label: 'Password',
      password_required: 'Enter your password to confirm.',
      confirm_button: 'Permanently delete my account',
      invalid_password: 'Incorrect password.',
      owns_organizations: 'Transfer or delete your organization(s) before deleting your account.',
      error_generic: 'Something went wrong. Try again.',
      confirm_dialog_message: "This is permanent and can't be undone. Confirm deletion?",
    },
  },
};

window.intaheLocale = function () {
  return document.documentElement.lang === 'en' ? 'en' : 'fr';
};

window.intaheLocaleTag = function () {
  return window.intaheLocale() === 'en' ? 'en-CA' : 'fr-CA';
};

window.intaheT = function (key, vars) {
  var dict = window.__INTAHE_I18N__[window.intaheLocale()];
  var value = key.split('.').reduce(function (acc, part) {
    return acc && Object.prototype.hasOwnProperty.call(acc, part) ? acc[part] : undefined;
  }, dict);
  if (typeof value !== 'string') return key;
  if (!vars) return value;
  return value.replace(/\{\{(\w+)\}\}/g, function (_, name) {
    return Object.prototype.hasOwnProperty.call(vars, name) ? String(vars[name]) : '';
  });
};

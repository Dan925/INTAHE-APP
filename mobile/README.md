...
### Testing the Quick Sale card reader (requires an EAS development build)

`@stripe/stripe-terminal-react-native` is a native module with its own config plugin (Bluetooth + location permissions) — like Sign in with Apple, it is **not** available in the plain Expo Go app, so it needs the same custom **EAS development build** as above. Unlike Sign in with Apple, it doesn't need any special Apple ID entitlement, so a free Apple ID is enough for an iOS build — the paid Apple Developer Program membership is still needed eventually only for the real App Store submission, not for testing this feature.
...
2. **The physical reader must already be registered to the organization's Stripe Terminal Location before the app can find it** — this app only *creates* the Location (via the "Configurer un lecteur" form on the Quick Sale screen) and *discovers/connects* to readers already registered to it; registering a specific physical unit's serial number to a Location is a one-time step done either in the Stripe Dashboard (Terminal → Readers → Register reader, using the code shown on the WisePOS E's own screen) or via `stripe.terminal.readers.create({ registration_code, location })`, and isn't something this app's code does. Do the Location setup in-app first, note the resulting location id, then register the physical WisePOS E to that same location.
3. The WisePOS E must be online (Wi-Fi/Ethernet) and the organization's Stripe account connected and charges-enabled — same "stripe_not_connected" gate as the rest of Quick Sale.
4. On the Quick Sale screen: set up the reader location (first time only), tap "Connecter un lecteur", select the discovered WisePOS E, then "Vendre" on a catalog item and tap/insert/swipe a [Stripe test card](https://docs.stripe.com/terminal/testing) on the reader.
5. Confirm: "Paiement réussi", then the sale appears in "Ventes récentes" as paid, with its instant payout status updating shortly after.

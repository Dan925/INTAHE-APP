...
- **Tickets (QR codes)**: a "Voir mes billets" screen that fetches the buyer's tickets for an order (`GET /v1/events/:eventId/orders/:orderId/tickets`, a buyer-facing endpoint — ownership is checked via the session or a per-order access token returned by checkout (`?token=...`, never `buyer_email` — see the backend README's "Ticket access token" section), never orderId alone) and renders each ticket's QR code (generated server-side as a PNG data URI via the `qrcode` package).
...
- **Quick Sale + card reader** (org detail screen's "Vente rapide" button): the same event-independent fixed-price catalog as the backend's Quick Sale module, plus a Stripe Terminal integration (`@stripe/stripe-terminal-react-native`) to take a sale on a physical reader (e.g. a WisePOS E) instead of typing a card on screen. First-time setup creates a Stripe Terminal "Location" for the organization (`POST .../quick-sale-reader/location`); discovery/connection use `discoveryMethod: 'internet'`, matching how a WisePOS E connects (over the internet, not a direct Bluetooth pairing). The reader connection UI itself (`src/components/card-reader-panel.tsx`) is shared with Door Sale below, since both need the exact same setup/discover/connect flow and one Location/reader serves an organization's whole event, not just Quick Sale. See "Testing the Quick Sale card reader" below — this is a native module, so it needs an EAS development build like Sign in with Apple does.
- **Door Sale** (event management screen's "Vente à la porte" button): sells a *real* ticket type — same order, same QR code, same webhook/ticket-issuance path as an online purchase — but charged on the organization's connected physical reader instead of Stripe.js/PaymentSheet (`POST /v1/organizations/:organizationId/events/:eventId/door-sales`, a staff-only endpoint distinct from the public buyer-facing checkout route). Staff pick a ticket type, enter the buyer's email and a quantity, then tap/insert/swipe the card on the connected reader.
...
### Testing the card reader — Quick Sale and Door Sale (requires an EAS development build)

`@stripe/stripe-terminal-react-native` is a native module with its own config plugin (Bluetooth + location permissions) — like Sign in with Apple, it is **not** available in the plain Expo Go app, so it needs the same custom **EAS development build** as above. Unlike Sign in with Apple, it doesn't need any special Apple ID entitlement, so a free Apple ID is enough for an iOS build — the paid Apple Developer Program membership is still needed eventually only for the real App Store submission, not for testing this feature. Quick Sale and Door Sale share the exact same reader-location/discover/connect UI (`card-reader-panel.tsx`), so everything below applies to both — connecting a reader once on either screen makes it usable on the other too, for as long as the app stays open (it disconnects when either screen unmounts).

1. Build and install a dev client the same way as "Testing Sign in with Apple" above (`npx eas build --profile development --platform ios` or `--platform android`), then `npx expo start --dev-client`.
2. **The physical reader must already be registered to the organization's Stripe Terminal Location before the app can find it** — this app only *creates* the Location (via the "Configurer un lecteur" form) and *discovers/connects* to readers already registered to it; registering a specific physical unit's serial number to a Location is a one-time step done either in the Stripe Dashboard (Terminal → Readers → Register reader, using the code shown on the WisePOS E's own screen) or via `stripe.terminal.readers.create({ registration_code, location })`, and isn't something this app's code does. Do the Location setup in-app first, note the resulting location id, then register the physical WisePOS E to that same location.
3. The WisePOS E must be online (Wi-Fi/Ethernet) and the organization's Stripe account connected and charges-enabled — same "stripe_not_connected" gate as the rest of Quick Sale/Door Sale.
4. Set up the reader location (first time only), tap "Connecter un lecteur", select the discovered WisePOS E.
   - **Quick Sale**: "Vendre" on a catalog item, then tap/insert/swipe a [Stripe test card](https://docs.stripe.com/terminal/testing) on the reader. Confirm: "Paiement réussi", the sale appears in "Ventes récentes" as paid, with its instant payout status updating shortly after.
   - **Door Sale**: from an event's management screen, "Vente à la porte" → pick a ticket type, enter a buyer email and quantity, "Vendre", then tap/insert/swipe the test card. Confirm: "Paiement réussi", and (once the webhook fires) the order shows up paid in that event's "Commandes" list with a real QR-coded ticket, just like an online purchase.

This flow needs a real WisePOS E reachable over the internet and a real device to run the dev client on — it cannot be verified from this sandbox (no route to Stripe's Terminal servers, no physical reader).
